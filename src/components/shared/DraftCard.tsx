...
export default function DraftCard({
    ...
    return (
        <div className={`steel-card ${theme} w-full aspect-video cursor-pointer group relative flex items-center justify-center overflow-hidden rounded-[8px] bg-[#0a0a0a] border border-white/5 transition-all duration-700 isolation-auto`}>

            {/* Холст с пикселями */}
            <PixelCanvas gap={4} speed={25} theme={theme} />

            {/* Внутренняя динамическая рамка */}
            <div className="inner-border absolute inset-[1px] rounded-[7px] border border-white/5 pointer-events-none z-10 transition-all duration-500 group-hover:opacity-100" />
